...
disp('- perform evaluation on each image')
...
disp('- collect evaluation results')
...
% oisCntRt=0; oisSumRt=0; oisCntPt=0; oisSumPt=0;
for i=1:n
  ...
  % oisCntPt=oisCntPt+cntP1(2); oisSumPt=oisSumPt+sumP1(2); % test
  % [Rt, Pt, Ft] = computeRPF(cntR1(k),sumR1(k),cntP1(k),sumP1(k)); % test
  % disp(Ft); % test
  % disp(k);
  % disp(F(k));
  % disp(F(2));
  % disp([cntR1(k),sumR1(k),cntP1(k),sumP1(k)]);
  % disp([cntR1(2),sumR1(2),cntP1(2),sumP1(2)]);
  % disp([cntP1(k)/sumP1(k),cntP1(2)/sumP1(2)]);
  % disp('next')
end

% compute ODS R/P/F and OIS R/P/F
disp('- collect compute ODS R/P/F and OIS R/P/F')
